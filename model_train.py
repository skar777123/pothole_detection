...
N_SAMPLES        = 4000
BASELINE_CM      = 180         # Default sensor mounting height (cm)
...
def generate_synthetic_data(n_samples=N_SAMPLES, window_size=WINDOW_SIZE,
    ...
    for cls, etype, depth_range in class_configs:
        for _ in range(per_class):
            ...
            w = _make_window(baseline, noise_std=NOISE_STD,
...
if __name__ == "__main__":
    ...
    print(f"  Baseline: {BASELINE_CM} cm  |  Window: {WINDOW_SIZE}  |  Features: {N_FEATURES}")
    print("=" * 60)

    print(f"\n[1/4] Generating {N_SAMPLES} synthetic windows …", flush=True)
